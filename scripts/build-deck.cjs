...
pptx.layout='LAYOUT_WIDE';pptx.author='LearnFlow 学习流学生团队';pptx.subject='本地原型与 Buddy 应用配置材料';pptx.title='学习流 - 自主学习支持平台';pptx.company='独立学生团队';pptx.lang='zh-CN';
...
if(d.kind==='cover'){s.addImage({path:cat,x:10.05,y:1.35,w:2.35,h:2.35});text(s,d.title,.7,1.45,8.6,.96,55,C.white,true);text(s,d.subtitle,.75,2.78,9.5,1.46,31,C.white,true);text(s,'基于可进化学习策略与腾讯 Buddy 生态的自主学习支持平台',.76,4.61,11.9,.45,17,'FFCC9B');d.chips.forEach((t,k)=>{rect(s,.76+k*3.53,5.63,3.22,.5,'2F3B48');text(s,t,.93+k*3.53,5.71,2.9,.28,14,'FFE2CB',true,{align:'center'});});text(s,'中国地质大学（北京） · 独立学生团队 · 2026.09',.77,6.4,11,.28,11,'B9C4CE');}
...
const dest=path.join(out,'LearnFlow-答辩演示.pptx');await pptx.writeFile({fileName:dest});console.log(JSON.stringify({pptx:dest,slides:data.length,screenshot:screenshot||null}));
})();
