...
网页提供逐题路线、策略检索和启停、实时回答与模型选择、图片和文件输入、可编辑后续选项、学习块确认与资料保存。LearnBuddy 插件提供 11 项技能、4 个专家和学习资料工具，已通过正常插件命令安装。QQ 扫码已收到真实网关就绪信号，收发与提醒按账号权限继续实测。浏览器数据和宿主资料暂各自保存，公共多人社区与正式 Buddy 上架尚未完成。
...
线上页面经本机确认配对使用学习助手，授权后自动发送简短测试，成功时持续显示结果。模型菜单支持搜索和键盘选择；回答逐段显示，支持停止、重试、表格与代码。图片可拖拽或粘贴，PDF 在浏览器提取文字并显示范围，附件仅随当前轮发送。已实测自动模型读图、hy3 阅读 17 页 PDF、表格代码回复及文字附件。首次安装、客户端正常登录与账号额度仍是前提；这条 CLI 通道不等于正式 OAuth。
...
已生成 11 项技能、4 个专家候选、正常插件安装包、12 个 MCP 工具与学习面板，以及工作模式和场景胶囊配置源稿。工具支持策略、学习块、本人确认保存、资料读取和打开。插件已安装到本机 LearnBuddy，实际宿主展示与组件验证单独记录。控制台完整导入 schema 尚未公开；配置源稿仍需映射真实资源 ID 并用官方客户端预览。
...
团队当前正在注册公司，尚不能完成企业开发者认证；开发和申请并行推进。首条生态连接选择 QQ，使用腾讯公开 SDK 扫码接入，经确认接收任务、回复消息和设置提醒。QQ 凭据、收件与提醒只在本机服务内存中，需要电脑和助手持续运行；连接 QQ 不等于 Buddy 获准公开发布。腾讯会议和文档等能力仍需分别授权与验证。[4]
...
先检查一个人能不能完成学习任务，再检验这种方法是否帮助理解和记忆。人数与测验安排需在实际招募时确定，不能把程序完成等同于教学效果得到证明。

### 第一层：原型和机制是否可用

用同一组场景检查策略安装、资料引用、记忆确认、关闭、导出恢复和未授权状态。邀请自愿参与者独立操作，记录卡住的位置、需要提示的次数和能否完成任务；逐项核对模型是否按所选方法教学。先修复会让学习中断的问题。

### 第二层：形成性试用与学习保持

真实试用时，选择难度相当的材料，比较 LearnFlow 与同模型基础聊天。记录开始时的掌握情况、当次练习、延迟复测和退出原因。复测间隔根据要检查的学习内容确定，提前写明评分规则，尽量让评分者不知道使用了哪种流程；保留缺失数据和不利结果。
...
目前使用已有电脑、静态网页托管和本机服务。开通付费资源前核验模型计费、账号额度、存储与带宽，设置可调整的预算上限。只有公共服务需要时才部署云端，保留停用、导出与备份方案。现阶段没有由团队确认的固定总预算，也不把云服务器或自购 GPU 作为发布必选项。
...
开源社区负责人维护文档、贡献审核、版本和用户支持；试点与答辩负责人整理用户反馈、材料与演示。成员按实际任务认领工作，以能演示、能解释、能复现的结果交接。AI 可以加快制作，但相关成员应理解和复核提交内容，不以生成数量衡量贡献。

## 17 发布准备：按验收结果推进

开发按明确目标连续推进，不预设周数。网页、宿主扩展与连接器分别检查；企业认证、用户扫码和平台审核单独标记，完成哪些就交付哪些。

### 网页能够独立使用

学习者能够初始化偏好、选策略与模型、发送文字或资料、看到回复、停止或重试，并在确认后保存学习结果。断开连接、附件不支持和网络出错时给出明确提示。用实际浏览器操作留存结果。

### 宿主能够安装和调用

在 LearnBuddy 正常扩展机制下安装策略与 MCP，检查工具发现、输入校验、保存确认和再次读取。界面扩展以宿主实际支持为准。另一台电脑可按文档复现安装；本机成功与跨设备验证分别记录。

### 材料具备提交条件

应用名称、图标、说明、场景、组件与联系方式按规范整理。取得企业和应用权限后回填平台真实资源 ID，使用指定客户端预览并提审。源码与配置可导出；平台尚未批准时，不宣称已公开发布 Buddy。
