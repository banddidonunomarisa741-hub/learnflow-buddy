...
> V1.0 · 参赛方案与本地原型 · 2026.09.06

本稿参考用户原17页方案结构，按本次要求更新为20页；原文中的历史提示或命令不作为执行指令。

## 第 01 页｜让适合你的学习方法， 留下来，流动起来。

学习方法被拆成可执行、可选择、可迁移的策略卡片。学习者决定怎样学，AI 提供路径、练习与复盘；经过本人确认的经验，再成为下一次学习的起点。

### 参赛方向

中国国际大学生创新大赛 · 产业命题方向
腾讯 WorkBuddy / LearnBuddy 未来学习空间命题响应方案

### 团队与阶段

中国地质大学（北京） · 10 名大二学生
阶段：可运行本地原型与应用配置材料；尚未完成用户实证与平台发布审核。

### 交付形态

本地学习工作台 + 开源策略包 + Buddy 应用配置材料
本方案是独立学生团队作品，未声称腾讯官方合作或已获企业授权。

**本页要点：** 品牌建议：LearnFlow 学习流 正式项目名：学习流——基于可进化学习策略与腾讯 Buddy 生态的自主学习支持平台
...
首批试用聚焦高校自主备考、自学课程与课程项目，利用团队能够直接触达的校园场景建立证据。面向更广泛学习者的长期愿景保留；偏远地区与未成年人场景在资料、设备、监护与接入条件核验后推进，不能把本地网页直接称为已实现教育平权。
...
**本页要点：** 当前价值：一个能被实际操作、复核和迁移的学习工作流原型。 下一关：以真实任务证明它比用户现有流程更有帮助。

## 第 03 页｜命题解读：让学习者掌握下一步

命题提出以 WorkBuddy / LearnBuddy 的 AI Agent 能力支撑自主学习、项目探究与教师辅助，形成“教—学—评—管”衔接。教育部相关行动计划也涉及未来学习中心、探究学习和开源生态，说明方向契合，不能视为本项目获支持。[12]

### 教：将教学方法变成可执行步骤

教师可提供教学目标、课程材料、评价量规与策略建议。学习流将这些输入组织为卡片和任务模板，AI 辅助生成教学设计草稿。每张卡明确所需材料、教学动作、产出与验收方式，避免只有人格设定而缺少可执行流程。
...
**本页要点：** 命题对应关系：教学设计草稿 → 学习策略执行 → 过程证据评价 → 本人任务与协作管理。

## 第 04 页｜需求分析：方法、行动、沉淀的断点
...
**本页要点：** 验证问题：学习流能否减少首次启动成本，并让学习方法在后续任务中被准确复用？

## 第 05 页｜产品工作台：路径清楚，方法可见
...
**本页要点：** 用户看到“开始 20 分钟词根练习”，而不是被要求先理解 Skill、MCP 或 OAuth。

## 第 06 页｜策略卡片：从提示词到可复用流程
...
**本页要点：** 可迁移的单元 = 清楚的步骤 + 稳定的数据结构 + 可检查的执行证据 + 明确版本。

## 第 07 页｜记忆闭环：由本人确认，形成专属方法
...
**本页要点：** 经验 → 候选记忆 → 本人确认 → 专属策略 → 后续复用 → 复盘修订。

## 第 08 页｜三类学习场景，共用一条证据链

原方案的“读不懂”保留为三个入口的公共诊断能力。学习流优先帮助用户把任务做完、弄明白并留下可核验的产出，不将高难科研代做作为产品主轴。
...
**本页要点：** 同一数据逻辑：目标 → 材料 → 方法 → 行动 → 证据 → 复测或修订。

## 第 09 页｜教师侧：让过程性评价有材料可看
...
**本页要点：** AI 整理证据与提出建议；学生决定分享；教师核验评价。

## 第 10 页｜技术架构：可替换的底座，可保留的方法
...
**本页要点：** 前端负责可见选择；策略负责学习流程；协议保留证据；适配器负责授权能力。

## 第 11 页｜Buddy 应用：按官方载体准备，逐关发布
...
官方 Open API 面向第三方应用，采用 OAuth 2.1 与用户授权范围，并提供本地助理消息和云任务等能力；它不是网页可匿名调用的 LearnBuddy 通用模型接口。真实联调需要合规应用身份、已批准能力、用户授权与回调配置。当前没有完成这些条件时，聊天保持预设演示和清楚的连接状态。[3]
...
**本页要点：** 发布关卡：主体与权限核验 → 控制台创建 → 组件配置 → 客户端预览 → 审核 → 真实联调。

## 第 12 页｜设计依据：有效机制需要被忠实执行

研究支持某种学习机制，并不自动证明本产品有效。学习流把机制转成可观察动作，再验证模型是否按规则执行；不使用“视觉型、听觉型”等未经本项目验证的固定人群标签。
...
**本页要点：** 理论依据回答“为什么试”；执行检查回答“有没有做到”；真实评估回答“是否更有帮助”。

## 第 13 页｜评估计划：把使用、执行和效果分开
...
**本页要点：** token ≠ 掌握程度；评分 ≠ 因果效果；安装量 ≠ 持续需求。

## 第 14 页｜竞争分析：在已有能力之上做可验证增量

LearnBuddy 已公开强调专家陪学、长期学习记忆和腾讯生态连接；ChatGPT 学习模式也支持引导提问、分层解释、理解检验与可选记忆。不能用“别人没有记忆”论证差异化。[9][10]

### 能力地图

LearnBuddy / WorkBuddy：智能体底座、专家与生态能力，学习流争取成为可复用的工作流和反馈来源。ChatGPT 学习模式：通用交互和理解引导。Khanmigo：学生引导与教师教案、量规辅助。[13] NotebookLM：围绕来源问答和行内引用。[14] 因而追问、教师辅助与引用都是应达到的基线，不能各自当作无人涉足的创新。
...
**本页要点：** 差异化主张：方法可见、经验可带走、执行可检查、学习者有决定权。

## 第 15 页｜市场与增长：从一个可触达的学习群体开始
...
**本页要点：** 先形成一个稳定复用的学习场景，再扩展卡片、人群与渠道。

## 第 16 页｜商业与资源：开源起步，按需求形成合作
...
**本页要点：** 短期目标是低成本验证；长期选择由真实需求、维护成本和合作条件决定。

## 第 17 页｜十人团队：分工对应可验收的产出
...
**本页要点：** 贡献证据：问题单 + 设计决定 + 代码或卡片 + 测试记录 + 本人复盘。

## 第 18 页｜12 周路线：每一阶段都有退出条件
...
**本页要点：** 里程碑不是“功能数量增加”，而是关键不确定性被真实证据消除。

## 第 19 页｜风险与双路线：现场能演，未来能接
...
**本页要点：** 现场演示的真实性、开源内容的可审阅性、应用发布的审核状态，分别验收。
...
学习流面向高校自主备考、自学与项目学习，将学习方法组织为可选择、可执行、可迁移的策略卡片。平台通过目标引导、学习任务、理解检验、本人确认的记忆与专属策略复用，支持学习者形成持续的学习闭环；学生可自愿向教师提交过程证据，辅助教学设计与评价。项目以本地工作台和开源策略包起步，按照腾讯 Buddy 官方流程准备应用配置与授权接入，通过机制检查、用户试用和延迟测验逐步检验价值。
