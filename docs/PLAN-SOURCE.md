...
线上网页经本机确认配对连接学习助手，授权后发送简短测试，成功时持续显示结果。对话框可选择客户端声明的模型，拖拽或粘贴图片，上传 PDF 与文字资料；图片按多模态消息发送，PDF 在浏览器提取文字并显示页数及截取范围。附件仅随当前轮发送，不自动保存。已实测自动模型读图和 hy3 阅读 17 页 PDF。首次安装与客户端正常登录仍是前提；正式 OAuth 审核尚未完成。
