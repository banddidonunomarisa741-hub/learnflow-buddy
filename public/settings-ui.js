(() => {
  ...
  const sections = [
    { id: 'general', label: '常规', icon: 'settings', group: '个人', description: '目标、学习节奏和交流方式，随时都能改。', terms: '目标 时间 引导 交流 鼓励 偏好 路线 初始化' },
    { id: 'appearance', label: '外观', icon: 'type', group: '个人', description: '选一个看着舒服的界面。', terms: '主题 深色 浅色 暗色 字体 大小 动画 显示' },
    { id: 'learning', label: '学习策略', icon: 'layers', group: '个人', description: '留下顺手的方法，其余的可以先关掉。', terms: '方法 skill 卡片 技能 启用 停用 token 评分 收藏' },
    { id: 'memory', label: '学习记忆', icon: 'heart', group: '个人', description: '只记住你确认过的事。', terms: '偏好 记住 保存 删除 导出 记忆' },
    { id: 'connections', label: '连接与模型', icon: 'plug', group: '集成', description: '连接模型和常用工具，授权由你决定。', terms: '接口 模型 API QQ 微信 腾讯 WorkBuddy LearnBuddy 本地 授权 连接器 提醒' },
    { id: 'teacher', label: '教师反馈', icon: 'clipboard', group: '集成', description: '把学习过程整理好，再决定分享给谁。', terms: '教师 老师 评价 证据 草稿 反馈' },
    { id: 'data', label: '数据与备份', icon: 'scan', group: '应用', description: '换设备前，记得把自己的学习记录带走。', terms: '导入 导出 清除 恢复 备份 数据 隐私 本地 存储' },
    { id: 'about', label: '关于 LearnFlow', icon: 'sprout', group: '应用', description: 'LearnFlow学习流动', terms: '关于 开源 github 版本 团队 logo' }
  ];
  ...
  window.LearnFlowSettings = {
    create(ctx) {
      ...
      function appearanceView() {
        const ui = readUI();
        return `<section class="lf-setting-section"><h2>主题</h2><div class="lf-theme-options" role="group" aria-label="界面主题">${[['light', '浅色', 'sun'], ['dark', '深色', 'moon'], ['system', '跟随系统', 'monitor']].map(([value, label, ic]) => `<button type="button" class="lf-theme-choice" data-ui-key="theme" data-ui-value="${value}" aria-pressed="${ui.theme === value}"><span class="lf-theme-preview" data-preview-theme="${value}" aria-hidden="true"><i></i><span><b></b><em></em><b></b><small></small></span></span><span class="lf-theme-caption">${glyph(ic)}${label}<span class="lf-choice-check">${ui.theme === value ? glyph('check') : ''}</span></span></button>`).join('')}</div></section><section class="lf-setting-section"><div class="lf-setting-row"><div><h2>对话文字大小</h2><p>只调整阅读字号，不会改变回答的内容。</p></div><div class="lf-segmented" role="group" aria-label="对话文字大小">${[['compact', '小'], ['normal', '标准'], ['large', '大']].map(([value, label]) => `<button type="button" data-ui-key="textSize" data-ui-value="${value}" aria-pressed="${ui.textSize === value}">${label}</button>`).join('')}</div></div><div class="lf-type-preview"><span>LearnFlow</span><p>先合上书，试着用自己的话讲一遍。卡住的地方，就是下一步。</p></div></section><section class="lf-setting-section"><label class="lf-setting-row"><span><strong>减少动态效果</strong><small>让过渡和滚动安静一些。</small></span><input class="toggle" type="checkbox" data-ui-motion ${ui.reducedMotion ? 'checked' : ''}></label></section><p class="lf-settings-footnote">外观设置会自动保存在此浏览器。</p>`;
      }

      function dataView() {
        const state = getState();
        const counts = [['个人策略', state.custom?.length || 0], ['学习记忆', state.memories?.length || 0], ['项目任务', state.tasks?.length || 0]];
        return `<section class="lf-setting-section"><h2>保存在这个浏览器</h2><p class="lf-settings-muted">学习偏好、策略和记录保存在本机。清理浏览器数据或更换设备后，需要用备份恢复。</p><div class="lf-data-counts">${counts.map(([label, count]) => `<div><strong>${count}</strong><span>${label}</span></div>`).join('')}</div></section><section class="lf-setting-section lf-data-actions"><div class="lf-setting-row"><div><h3>导出学习数据</h3><p>下载一份备份，包含偏好、策略、记忆和已保存的记录。</p></div><button type="button" class="secondary" data-action="export-all">${glyph('download')}导出备份</button></div><div class="lf-setting-row"><div><h3>从备份恢复</h3><p>先预览备份内容，再确认是否替换当前数据。</p></div><button type="button" class="secondary" data-action="restore-backup">${glyph('upload')}选择备份</button></div><div class="lf-setting-row"><div><h3>导出学习策略</h3><p>把已启用的方法带到另一台设备。</p></div><button type="button" class="secondary" data-action="export-strategies">导出策略</button></div></section><section class="lf-setting-section"><div class="lf-setting-row"><div><h3>清除本机学习数据</h3><p>操作前还会让你确认，建议先导出一份备份。</p></div><button type="button" class="secondary danger" data-action="clear-data">${glyph('trash')}清除数据</button></div></section><p class="lf-settings-footnote">备份文件可能包含个人学习记录，分享前请检查内容。学习资产的原始文件请另行保存。</p>`;
      }
      ...
      function contentView(section) {
        ...
        return `${section === 'general' ? `<div class="lf-settings-start"><span>想重新整理一下学习目标？</span><button type="button" class="text-link" data-action="onboard">重新建立路线 ${glyph('arrow')}</button></div>` : ''}<div class="lf-settings-embedded" data-embedded-section="${section}">${view ? view() : ''}</div>`;
      }
