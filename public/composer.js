...
(() => {
  ...
  function openModels() {
    ...
    menu.innerHTML = `<div class="model-menu-heading"><h2 id="model-menu-title">选择模型</h2><button type="button" id="close-models" aria-label="关闭模型选择">${svg('close')}</button></div><label class="model-search-field">${svg('search')}<input id="model-search" type="search" autocomplete="off" placeholder="搜索模型" aria-label="搜索模型"></label><div class="model-list-meta"><span id="model-count"></span></div><div id="model-options" role="listbox" aria-label="模型列表"></div><div class="model-menu-footer"><button type="button" id="refresh-models">${svg('refresh')}<span>刷新列表</span></button><button type="button" id="model-connection-settings">连接设置</button></div>`;
