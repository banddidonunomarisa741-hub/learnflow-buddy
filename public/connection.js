...
(() => {
  ...
  async function open() {
    ...
    if (!h) {
      dialog.insertAdjacentHTML('beforeend', `<p>线上页面需要本机连接助手。首次下载并解压后，双击“安装连接助手.cmd”；以后可直接点击下方按钮打开。</p><p><a class="primary" href="./downloads/learnflow-connector.zip" download>下载 Windows 连接助手</a></p><p><a class="secondary" href="learnflow://connect">已安装：打开本机 LearnFlow</a> <a href="http://127.0.0.1:4173/#session" target="_blank" rel="noopener">服务已启动：进入学习</a></p><p class="caption">Windows 或浏览器可能要求确认打开应用。首次安装需要 Node.js。线上与本机的学习记录分别保存在各自浏览器空间，可用“导出 / 恢复备份”迁移。打开客户端本身不代表取得模型调用授权。</p>`);
    } else {
      dialog.insertAdjacentHTML('beforeend', `<p id="connection-status" role="status">${h.configured?'接口已配置；真实回复成功后才能确认可用。':'连接助手已启动，请选择接口。'}</p><form id="connection-form"><label>连接方式<select name="provider" class="wide-button">${h.localCLIAvailable?'<option value="local-codebuddy">自动连接本机 WorkBuddy 附带的 CodeBuddy</option>':''}<option value="openai-compatible">自己的模型接口（OpenAI 兼容）</option><option value="workbuddy-localassistant">WorkBuddy 官方授权通道</option></select></label><p id="cli-fields" hidden>已检测到 WorkBuddy 附带的 CodeBuddy。使用该入口自身的正常认证，不复制登录密钥；禁用文件、命令与 MCP 工具。版本更新可能改变此入口，连接失败可切换正式授权通道。</p><div id="compatible-fields"><p><label>接口地址<input class="wide-button" name="base" placeholder="https://你的服务/v1" autocomplete="off"></label></p><p><label>模型名称<input class="wide-button" name="model" placeholder="服务商提供的模型名" autocomplete="off"></label></p><p><label>API Key（本机无密钥服务可留空）<input class="wide-button" name="key" type="password" autocomplete="new-password"></label></p></div><div id="buddy-fields" hidden><p>须先完成官方第三方应用 OAuth 授权。本原型暂未取得可供所有用户一键登录的腾讯应用身份，不能自动复用桌面登录态。LearnBuddy 暂未验证独立公共调用接口。</p><p><a href="https://open.workbuddy.cn/docs/third-party-app" target="_blank" rel="noopener">查看官方授权说明 ↗</a></p><label>已合法取得的 WorkBuddy 访问令牌<input class="wide-button" name="token" type="password" autocomplete="new-password"></label><p>请先打开并登录 WorkBuddy；宿主可能要求你在客户端确认任务。</p></div><p><label><input type="checkbox" name="consent" required> 我同意发送上述学习内容；密钥仅保留到连接助手退出或我断开连接。</label></p><button class="primary" type="submit">同意并连接</button> <button class="secondary" type="button" id="connection-disconnect">断开并清除密钥</button></form>`);
      const form = dialog.querySelector('form');
      form.elements.provider.onchange = () => { const buddy = form.elements.provider.value === 'workbuddy-localassistant'; dialog.querySelector('#compatible-fields').hidden = form.elements.provider.value !== 'openai-compatible'; dialog.querySelector('#cli-fields').hidden = form.elements.provider.value !== 'local-codebuddy'; dialog.querySelector('#buddy-fields').hidden = !buddy; };
      ...
      form.onsubmit = async e => {
        ...
        try {
          ...
          form.elements.key.value = form.elements.token.value = '';
