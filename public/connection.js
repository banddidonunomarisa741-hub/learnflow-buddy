(() => {
 ...
 function proof(data){
  ...
  const tokens=data.usage?.source==='provider'&&Number.isFinite(data.usage.total_tokens)?data.usage.total_tokens.toLocaleString():'未提供';
  dialog.insertAdjacentHTML('beforeend',`<section class="connection-proof"><h3>✓ 已连接 ${esc(data.clientName||'模型服务')}</h3><dl><dt>实际模型</dt><dd>${esc(data.model||'服务未提供')}</dd><dt>测试用量</dt><dd>${tokens} ${tokens==='未提供'?'':'Tokens'}</dd><dt>本轮积分</dt><dd>当前通道未提供</dd></dl><details class="connection-proof-details"><summary>查看验证记录</summary><dl><dt>请求模型</dt><dd>${esc(data.requestedModel||data.model)}</dd><dt>响应耗时</dt><dd>${(Number(data.latencyMs||0)/1000).toFixed(1)} 秒</dd><dt>验证时间</dt><dd>${esc(new Date(data.checkedAt||Date.now()).toLocaleString())}</dd></dl><pre>${esc(data.response||'已收到校验回答。')}</pre><p>${esc(data.billing?.note||'当前通道未返回积分扣费或余额；以客户端账户账单为准。')} Token 用量不等于积分。</p></details></section>`);
 }
 function showReady(){
  button.textContent='✓ 模型已实际回复';dialog.classList.add('is-verified');
  const form=dialog.querySelector('#connection-form');if(form)form.hidden=true;
  status('✓ 已连接，可以开始学习。');
  if(!dialog.querySelector('#begin-connected'))dialog.insertAdjacentHTML('beforeend','<div class="connection-actions"><button class="primary" id="begin-connected">开始学习 →</button><button class="secondary" id="change-client-again">换一个入口</button></div>');
  dialog.querySelector('#begin-connected').onclick=()=>{dialog.close();location.hash='session';document.querySelector('#chat-input')?.focus();};
 }
 ...
 async function open(forceConfig=false){
  ...
  status(h?.configured?'已有访问许可；请测试当前选择的模型。':'连接后会用 Auto 发送一次简短测试。');
  const previous=h?.verification;if(previous?.verified&&previous.requestedModel===(window.LearnFlowComposer.model()||'auto')){proof(previous);if(!forceConfig)showReady();}if(!dialog.open)dialog.showModal();
 }
 dialog.addEventListener('cancel',()=>verificationAbort?.abort());
 dialog.addEventListener('click',event=>{if(event.target.closest('#change-client-again'))open(true);});
