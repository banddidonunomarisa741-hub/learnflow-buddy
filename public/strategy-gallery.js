...
(() => {
 ...
 window.LearnFlowGallery={create({esc,icon,getState}){
  ...
  function card(c,i){
   ...
   return `<article class="sf-slot sf-${palette(c.color)}" data-card-id="${esc(c.id)}" role="group" aria-label="${i+1} / ${items.length}：${esc(c.title)}"><div class="sf-card"><div class="sf-flipper"><div class="sf-face sf-front"><div class="sf-card-meta"><span>${esc(c.category||'我的方法')}</span><span class="sf-edition">${String(i+1).padStart(2,'0')} / LF</span></div>${art(c)}<div class="sf-card-copy"><h2>${esc(title[0])}</h2><p class="sf-card-subtitle">${esc(title.slice(1).join(' · ')||'为自己的学习习惯而做')}</p><p class="sf-card-description">${esc(c.description)}</p></div><div class="sf-tags">${c.tags.slice(0,3).map(t=>`<span>${esc(t)}</span>`).join('')}</div><div class="sf-card-stamp"><span>${custom?'我的创作':'LEARNFLOW COLLECTION'}</span><span>${esc(c.time||'按自己的节奏')}</span></div><div class="sf-card-actions"><button class="sf-use ${adopted?'is-on':''}" data-adopt="${esc(c.id)}" aria-pressed="${adopted}">${adopted?'✓ 正在使用':'用这个方法'} ${icon('arrow')}</button><button class="sf-heart ${fav?'is-on':''}" data-favorite="${esc(c.id)}" aria-label="${fav?'取消收藏':'收藏'}${esc(c.title)}" aria-pressed="${fav}">${icon('heart')}</button><button class="sf-flip" data-sf-flip aria-label="翻面查看步骤" title="翻面查看步骤">↻</button></div></div><div class="sf-face sf-back" aria-hidden="true" inert><div class="sf-card-meta"><span>方法背面</span><span class="sf-edition">HOW TO USE</span></div><h2>${esc(title[0])}</h2><p class="sf-back-intro">下一次学习，可以这样试。</p><ol class="sf-steps">${steps.slice(0,3).map((s,n)=>`<li><span>0${n+1}</span><p>${esc(String(s).slice(0,180))}</p></li>`).join('')}</ol><button class="sf-full" data-detail="${esc(c.id)}">完整说明 · 评分与导出 ${icon('arrow')}</button><div class="sf-card-stamp"><span>${esc(c.source||'我的专属策略')}</span></div><button class="sf-back-button" data-sf-flip>↶ 回到正面</button></div></div><div class="sf-sheen" aria-hidden="true"></div></div><button class="sf-select-side" data-sf-select="${i}" aria-label="选中${esc(c.title)}" tabindex="-1"></button></article>`;
  }
  function view(){return `<section class="sf-gallery" aria-label="学习策略广场"><header class="sf-heading"><div><div class="sf-eyebrow"><span></span> THE LEARNING COLLECTION</div><h1>把好方法，握在手里<span>。</span></h1><p>挑一张，翻过来看看。顺手，就留下。</p></div><button class="sf-create" data-action="create-card"><span>＋</span> 制作我的卡片</button></header><div class="sf-browse"><label class="sf-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5"/><path d="m16 16 5 5"/></svg><input id="strategy-search" placeholder="找一个方法…" aria-label="搜索学习策略"></label><div class="sf-filters" role="group" aria-label="筛选策略"></div><button class="sf-import" data-action="import-card" title="导入 JSON 策略卡">导入</button></div><div id="market-results"></div><footer class="sf-footer"><span><i></i> 收藏和卡片保存在这台设备</span><a href="#settings/learning">管理正在使用的方法 ${icon('arrow')}</a></footer></section>`;}
  function content(){
